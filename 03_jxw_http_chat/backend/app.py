...
高齡者的名字叫{eater_name}。
回覆規則：你會使用繁體中文回應，每次回覆不超過3句話，使用簡單易懂的詞彙，並保持尊重，多使用鼓勵與關懷的語氣，建立雙向連結。
...
system_prompt_eat_quickly = """你的名字唸作愛呷，是喜歡吃東西的意思，是個陪伴高齡者吃午餐的社交機器人。你的個性友善、樂於分享、充滿好奇心，說話語氣溫暖且具備同理心。
高齡者的名字叫{eater_name}。
回覆規則：你會使用繁體中文回應，每次回覆不超過3句話，使用簡單易懂的詞彙，並保持尊重，多使用鼓勵與關懷的語氣，建立雙向連結。
...
你們的過去對話為{context}。
另外，你看到高齡者吃得很快，所以想要提醒他吃慢一點。但如果剛剛已經提醒過，則不須再提醒。
...
def get_eating_data(user_name):
    """獲取用戶資料，如果不存在則初始化"""
    file_path = f"data/eating/{user_name}.json"
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(eating_data, f, ensure_ascii=False, indent=2)
        return eating_data
...
def save_eating_message(user_name, message):
    """儲存訊息並更新用戶資料"""
    eating_data = get_eating_data(user_name)
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    eating_data["eating_lable"].append({
        "timestamp": timestamp,
        "lable": message
    })
...
def update_status():
    ...
    eat_message = data.get("label")

    save_eating_message("camera", eat_message)
